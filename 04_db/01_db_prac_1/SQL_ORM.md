...
2. user 레코드 생성
   ...
   ```sql
   -- sql
   INSERT INTO users_user VALUES (102,'길동','김',200,'경
   상북도','010-1234-5678',20000);
   ```
...
7. 지역번호가 02인 사람의 인원 수
   ...
   ```python
   # orm
   User.objects.filter(phone__startswith='02').values().count()
   ```
...
```
      ...
      ```sql
   -- sql
   SELECT first_name FROM users_user WHERE country='강원
   도' and last_name='황';
      ```
...
3. 강원도에 사는 사람의 평균 계좌 잔고
   ...
   ```sql
   -- sql
   SELECT AVG(balance) FROM users_user WHERE country='강
   원도';
   ```
